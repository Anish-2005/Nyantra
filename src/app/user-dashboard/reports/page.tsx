  ...
  return (
    <div data-theme={theme} className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full blur-3xl" style={{ backgroundColor: theme === 'dark' ? '#1e40af' : '#3b82f6' }}></div>
        <div className="absolute bottom-0 right-0 w-96 h-96 rounded-full blur-3xl" style={{ backgroundColor: theme === 'dark' ? '#7c3aed' : '#8b5cf6' }}></div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm theme-text-primary">{t('extracted.filters')}:</span>
            </div>
        ...
        {filteredReports.map((report, idx) => (
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              ...
              <div className="text-center p-2 rounded-lg theme-bg-glass">
                <div className="flex items-center justify-center gap-1 mb-1">
                  {(() => {
                    const FormatIcon = getCategoryIcon(report.fileFormat.toLowerCase()) || FileText;
            ...
            <div className="flex items-center justify-between pt-3 border-t theme-border-glass">
              <div className="flex items-center gap-2">
                <span className="text-xs theme-text-muted">{t('extracted.type')}: {report.type}</span>
              </div>
              <motion.button
        ...
        {selectedReport && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl theme-bg-card theme-border-glass border shadow-2xl"
            >
              {/* Header */}
              <div className="flex items-center justify-between p-6 border-b theme-border-glass">
                <div>
                  ...
                  <p className="text-sm theme-text-muted mt-1">{selectedReport.id}</p>
                </div>
                <button
              ...
              <div className="p-6">
                ...
                {/* Status and Download */}
                <div className="mt-6 flex items-center justify-between p-4 rounded-lg theme-bg-glass theme-border-glass border">
                  <div>
                    <h4 className="text-sm font-semibold theme-text-primary mb-1">{t('extracted.status')}</h4>
                    <p className="text-sm theme-text-muted">{t('extracted.current_status')}: <span className={`font-medium ${getStatusColor(selectedReport.status)}`}>{selectedReport.status}</span></p>
                  </div>
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
      ...
      {filteredReports.length === 0 && (
        ...
        >
          ...
          <h3 className="text-lg font-semibold theme-text-primary mb-2">{t('extracted.no_reports_found')}</h3>
          <p className="theme-text-muted">{t('extracted.try_adjusting_filters')}</p>
        </motion.div>
      )}
      </div>
    </div>
