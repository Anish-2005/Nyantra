...
import { collection, query, orderBy, onSnapshot, doc, getDoc, getDocs, serverTimestamp, setDoc, updateDoc, arrayUnion, type QueryDocumentSnapshot, type DocumentData } from 'firebase/firestore';
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <div className="w-10 h-10 rounded-md bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white">
                          <Database className="w-5 h-5" />
                        </div>
                        <div>
                          <h4 className="font-semibold theme-text-primary">{t('extracted.comprehensiveExport') || 'Comprehensive System Export'}</h4>
                          <p className="text-xs theme-text-muted">{t('extracted.comprehensiveExportDescription') || 'Download complete system data including applications, disbursements, beneficiaries, and grievances.'}</p>
                        </div>
                      </div>
                      <p className="text-sm theme-text-muted">{t('extracted.fullSystemData') || 'Complete system dataset'}</p>
                    </div>
                      ...
                      >
                        {loading ? (
                          <>
                            ...
                            {t('extracted.generating') || 'Generating...'}
                          </>
                        ) : (
                          <>
                            <Download className="w-4 h-4" />
                            {t('extracted.exportFullReport') || 'Export Full Report'}
                          </>
                        )}
                      </button>
                      <p className="text-xs theme-text-muted text-right">
                        {t('extracted.includesPDFAndCSV') || 'Includes PDF report + CSV data'}
                      </p>
                    </div>
                  </div>
                </div>
