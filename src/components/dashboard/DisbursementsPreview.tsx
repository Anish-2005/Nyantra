...
export default function DisbursementsPreview({ disbursements, loading }: DisbursementsPreviewProps) {
    const { t } = useLocale();
                ...
                <div className="space-y-3">
                    {loading ? (
                        ...
                        disbursements.map((disbursement, idx) => {
                            ...
                            return (
                                <motion.div
                                    key={disbursement.id + idx}
                                    className="relative p-4 rounded-xl theme-bg-glass border theme-border-glass flex items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors"
                                ...
                                >
                                    ...
                                    <div className="flex-1 pl-4">
                                        <div className="flex items-center justify-between mb-2">
                                            <div>
                                                <p className="font-semibold theme-text-primary text-sm">{disbursement.name}</p>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${disbursement.status === 'completed' ? 'bg-green-500/10 text-green-500' : 'bg-blue-500/10 text-blue-500'
                                                }`}>
